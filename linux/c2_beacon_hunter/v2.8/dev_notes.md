...
**Role:** Lead Developer
**Scope:** `v2.8/` Directory Iterations (2.8.1 - 2.8.3)
...
## Technical Debt & Defect Resolution
* **Defect:** SQLite Startup Race Condition
    * *Resolution:* Refactored database initialization in `c2_beacon_hunter.py` to assert a `CREATE TABLE IF NOT EXISTS` block, resolving the race condition between the collector and the UEBA learner upon fresh container deployments.
* **Defect:** File I/O Lock Contention
    * *Resolution:* Shifted `anomalies.jsonl` export logic strictly to append mode (`"a"`), guaranteeing compatibility with the newly integrated `c2_defend.py` daemon tailer.
* **Infrastructure:** Docker Compose Updates
    * *Resolution:* Expanded `docker-compose.yaml` to deploy the `api-dashboard` container alongside the core `c2-beacon-hunter` container, sharing read-only volume mounts (`data/`, `output/`, `certs/`) to ensure the dashboard can read the SQLite and JSON logs safely.
