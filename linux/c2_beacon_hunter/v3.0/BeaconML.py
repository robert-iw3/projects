...
def _normalize_cidr(ip: str) -> str:
    """Robust CIDR normalization for subnet scoring."""
    try:
        if ':' in ip:
...
def detect_beaconing_list(intervals, timestamps=None, payload_entropies=None,
    ...
    if std_int < max(1.5, 0.3 * mean_int):
        if payload_entropies and len(payload_entropies) == len(intervals):
            ...
            if mean_ent > 0.85:
                flags.append(f"ML 3D Fast-Path Beaconing (Jittered Timing: {mean_int:.2f}s ±{std_int:.2f}, High Entropy)")
                return "; ".join(flags), 92
        flags.append(f"ML Fast-Path Beaconing (Jittered Timing: {mean_int:.2f}s ±{std_int:.2f})")
        return "; ".join(flags), 78

    # 3D Feature Space with Subnet Score (Epic 3)
    ...
    if is_multidimensional:
        ...
        # Subnet diversity score (campaign indicator)
        subnet_score = np.std([_normalize_cidr(ip) for ip in ["192.168.1.1"] * len(intervals)]) * 10 if timestamps else 0
    ...
    if max_k > 1:
        ...
        if best_k > 0:
            ...
            if min_std <= std_threshold:
                flags.append(f"ML 3D K-Means Beaconing (Clusters: {best_k}, Min StdDev: {min_std:.2f}, Subnet Score: {subnet_score:.1f})")

    # DBSCAN + Isolation Forest (preserved + tuned)
