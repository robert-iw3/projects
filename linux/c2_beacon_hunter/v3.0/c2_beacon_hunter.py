...
v3.0 Epic 1 Addition:
- Dual-mode support (mode = host | promisc) with zero breakage to existing code
"""
...
# v3.0 Epic 1: Read mode (default host for full backward compatibility)
MODE = config.get('general', 'mode', fallback='host').strip().lower()
print(f"[v3.0 Epic 1] Operating in {MODE.upper()} mode")
...
if USE_EBPF:
    try:
        ...
        from collector_factory import get_collector   # now mode-aware (v3.0)
    except ImportError:
...
class BeaconHunter:
    def __init__(self, output_dir=OUTPUT_DIR):
        ...
        if USE_EBPF:
            ...
            self.ebpf_collector = CollectorFactory.create_collector()  # now mode-aware
