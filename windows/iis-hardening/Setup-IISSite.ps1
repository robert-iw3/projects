...
function Run-AppCmd {
    param([string]$Args, [bool]$IgnoreError=$false)
    # Using Start-Process ensures we wait for the lock file to release before moving to the next command
    $p = Start-Process -FilePath $AppCmd -ArgumentList $Args -Wait -NoNewWindow -PassThru
    if ($p.ExitCode -ne 0 -and -not $IgnoreError) {
        Throw "AppCmd failed with Exit Code $($p.ExitCode). Command: $Args"
    }
}
...
try {
    ...
    Run-AppCmd "set config `"$SiteName`" /section:httpProtocol /customHeaders.remove.[name='X-Powered-By']" $true
