...
// Constants
const MAX_EVENTS: usize = 512;  // Tuned for balance
const TAG_CONTEXT: u32 = u32::from_le_bytes(*b"monC");

// ML-Ready Event Struct (Fixed-point score adopted)
#[repr(C, packed)]
#[derive(Clone, Copy)]
pub struct MonitorEvent {
    ...
    path_len: u16,
    path: [WCHAR; 260],
    anomaly_score_fixed: u32,  // Adopted: Score * 1000 (e.g., 850 = 0.85)
    syscall_id: u32,
    event_id: u64,
    hash: [u8; 32],
    activity_id: GUID,
    cloud_context: [0u8; 64],
}

// Stream Context (Adopted for caching)
#[repr(C)]
...
// Globals (Atomic head/tail adopted)
...
// Optimized Log Event (Adopted short lock + atomic)
...
// Process Callback (Adopted hollowing detection)
...
// Thread Callback (Adopted injection detection)
#[cfg(feature = "threads")]
...
// Minifilter Pre-Op (Adopted stream context caching)
unsafe extern "system" fn pre_operation_callback(data: *mut FLT_CALLBACK_DATA, obj: PFLT_RELATED_OBJECTS, _: *mut PVOID) -> FLT_PREOP_CALLBACK_STATUS {
    ...
    if NT_SUCCESS(FltGetStreamContext((*obj).Instance, (*obj).FileObject, &mut ctx as *mut _ as *mut PFLT_CONTEXT)) {
        ...
        if NT_SUCCESS(FltGetFileNameInformation(data, FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT, &mut name_info)) {
            FltParseFileNameInformation(&mut name_info);
            log_event(op, PsGetCurrentProcessId(), core::ptr::null_mut(), name_info.Name.Buffer, name_info.Name.Length, 0);

            if NT_SUCCESS(FltAllocateContext(FILTER_HANDLE, FLT_STREAM_CONTEXT, size_of::<MonitorStreamContext>() as u64, NonPagedPoolNx, &mut ctx as *mut _ as *mut PFLT_CONTEXT)) {
...
// Context Cleanup Callback (New: Adopted for safety)
unsafe extern "system" fn context_cleanup_callback(context: PFLT_CONTEXT, _type: FLT_CONTEXT_TYPE) {
    if _type == FLT_STREAM_CONTEXT {
        // No-op: Static array, no free needed
    }
}

// Object Callback (Adopted injection detection)
#[cfg(feature = "objects")]
unsafe extern "system" fn object_callback(_: PVOID, pre_info: *mut OB_PRE_OPERATION_INFORMATION) -> OB_PREOP_CALLBACK_STATUS {
    if (*pre_info).ObjectType == *PsProcessType {
        ...
        if (access & 0x0028) == 0x0028 {  // VM_WRITE | VM_OPERATION
...
// ETW Tracing (unchanged)
unsafe fn trace_etw_event(event: &MonitorEvent) {
    if ETW_REG_HANDLE != 0 {
        EventActivityIdControl(EVENT_ACTIVITY_CTRL_SET_ID, &event.activity_id);
        ...
        EventWrite(ETW_REG_HANDLE, core::ptr::null(), 1, &mut desc);
    }
}

// Get Activity ID (unchanged)
unsafe fn get_current_activity_id() -> GUID {
    let mut id = CURRENT_ACTIVITY_ID;
    if id.data1 == 0 {
        RtlRandomEx(&mut id.data1 as *mut u32, 0);
        CURRENT_ACTIVITY_ID = id;
    }
    id
}

// Driver Entry (Adopted WDM elements + context cleanup)
...
pub unsafe extern "system" fn driver_entry(
    ...
    status = PsSetCreateProcessNotifyRoutineEx(Some(process_notify_callback), FALSE as BOOLEAN);
    ...
    // Minifilter with context cleanup
    ...
    if !NT_SUCCESS(status) { FltUnregisterFilter(FILTER_HANDLE); return status; }
...
// Unload (Adopted cleanup for contexts)
unsafe extern "system" fn unload_callback(_flags: FLT_REGISTRATION_FLAGS) -> NTSTATUS {
    PsSetCreateProcessNotifyRoutineEx(Some(process_notify_callback), TRUE as BOOLEAN);
    #[cfg(feature = "threads")]
    ...
    #[cfg(feature = "network")]
    FwpsCalloutUnregisterById0(WFP_CALLOUT_ID);
    #[cfg(feature = "ai_agent")]
    KeClearEvent(&mut AGENT_EVENT);
    STATUS_SUCCESS
}

// Operation Registration (unchanged)
...
// Context Registration (Adopted with cleanup)
static CONTEXT_REG: [FLT_CONTEXT_REGISTRATION; 2] = [
    ...
    FLT_CONTEXT_REGISTRATION { ContextType: FLT_CONTEXT_END, Flags: 0, ContextCleanupCallback: None, Size: 0, PoolTag: 0, ContextAllocateCallback: None, ContextFreeCallback: None, Reserved1: 0 },
];